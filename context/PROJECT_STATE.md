...
- **Two-Tier Transparent Reverse Proxy & ISP DNS Bypass Architecture:** Resolved client-side "Unable to connect to the storage server" errors caused by Indian ISP DNS resolvers (e.g. Reliance Jio / Airtel) failing to resolve `*.up.railway.app` (`ERR_NAME_NOT_RESOLVED`). Implemented a resilient two-tier proxy:
  1. *Edge Rewrites (`next.config.js`):* Automatically rewrites `/api/v1/telegram/:path*`, `/api/v1/shares/:path*`, and `/api/health` to `https://bucketspace-production.up.railway.app` when deployed without direct local credentials.
  2. *Serverless Route Fallback (`route.ts` handlers):* In all Telegram auth and MTProto API routes, transparently forwards requests upstream to the Railway backend if local credentials are absent, relaying responses directly.
  3. *Client-Side Origin Adapter (`src/lib/utils.ts`):* `normalizeApiBase` detects `*.vercel.app` in the browser and routes requests through relative paths (`""`) to Vercel's same-origin proxy, ensuring the user's browser never directly queries the blocked `*.up.railway.app` domain.
