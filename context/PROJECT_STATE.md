...
- **GitHub Actions CI pnpm Version Specification:** Configured `version: 10` in `pnpm/action-setup` in `.github/workflows/ci.yml` and declared `packageManager: pnpm@10.5.2` in `package.json` to resolve the 5-second runner failure.
- **Telegram Entity Resolution & Access Hash Architecture:** Resolved GramJS `Could not find the input entity for PeerUser` during public share downloads. Updated `uploadChunk` to preserve `channelId`, `channelAccessHash`, and proper `-100` channel ID prefixes in `TelegramRefData`. Implemented multi-tier entity resolution in `TelegramAuthService.downloadChunk` utilizing `InputPeerChannel` when access hash is available, automatically resolving bare numeric IDs against authenticated user IDs and the vault channel, and falling back gracefully across the storage vault and Saved Messages without ever throwing unhandled GramJS entity lookup exceptions.
- **Dual-Format Whole-File Cryptographic Integrity Verification:** Supported both raw file content digests and chunk-tree Merkle digests (`calculateSha256(chunkHashes.join(':'))`) in public share client verification, resolving false-positive whole-file hash mismatch errors while preserving zero-knowledge per-chunk and whole-file authentication.
- **Type-Safe In-Memory Test Fallback & ShareRecord Model Integrity:** Resolved Vercel production build TypeScript failure (`Property 'inMemoryChunks' does not exist on type 'ShareRecord'`). Strictly modeled `InMemoryChunkData` in `ShareRecord` and refactored the fallback in `GET /api/v1/shares/[token]/chunks/[index]` with type guards and explicit `Uint8Array<ArrayBuffer>` construction without weakening the model with `any` or `@ts-ignore`.
- **Cross-Origin Resource Sharing (CORS) Middleware Enhancement:** Configured `src/middleware.ts` to allow Vercel origins (`*.vercel.app`, `https://bucket-space.vercel.app`) and local development ports by default, returning origin reflection on preflight OPTIONS and request handlers, eliminating `Failed to fetch` and false connection errors during authentication flows from the Vercel frontend to the Railway MTProto backend.
- **Runtime Serverless Reverse Proxy & ISP DNS Bypass Architecture:** Resolved client-side "Unable to connect to the storage server" errors caused by Indian ISP DNS resolvers (e.g. Reliance Jio / Airtel) failing to resolve `*.up.railway.app` (`ERR_NAME_NOT_RESOLVED`).
  1. *Purged Static Rewrites from `next.config.js`:* Eliminated Next.js build-time rewrite rules that previously baked external Railway URLs into standalone Docker containers and caused infinite self-referencing loops and Hikari 500 health check failures on Railway.
  2. *Runtime Serverless Route Proxying (`route.ts` handlers):* Telegram authentication and MTProto chunk routes in Vercel serverless detect runtime absence of local Telegram credentials and forward requests upstream to the Railway container (`https://bucketspace-production.up.railway.app`), reading raw text payloads and preserving diagnostic error messages.
  3. *Client-Side Same-Origin Adapter (`src/lib/utils.ts`):* `normalizeApiBase` returns `""` when running in the browser on `*.vercel.app`, routing all traffic through Vercel's unblocked origin.
- **Mobile-Responsive Design Optimization (Desktop Layout Preserved):** Enhanced mobile usability across landing page, onboarding modal, file cards, and bulk action bar using strictly mobile breakpoint classes (`sm:`, `md:`) to leave laptop and desktop layouts completely untouched. Included touch-friendly action buttons, responsive typography, and mobile bottom sheets.
- **Telegram Vault Metadata Registry & Multi-Origin Auto-Sync Architecture (`GET/POST /api/v1/telegram/vault/sync`):**
  - *Problem Solved:* Personal drive file metadata was previously stored solely in browser `localStorage`. When users switched origins (e.g., from `http://localhost:3000` to `https://bucket-space.vercel.app`), cleared their browser cache, or logged in on a new laptop or smartphone, their files appeared missing even though all binary chunks remained safely stored in Telegram.
  - *Vault Registry Discovery & Reconstruction (`TelegramAuthService.syncVaultFiles`):*
    1. Scans the user's `📦 BucketSpace Vault` channels for an active `[bucketspace-registry-v1]` manifest message or attached JSON document.
    2. If no registry exists yet, automatically parses and reconstructs files from all `[bucketspace-chunk:chunk-${fileId}-${index}]` messages in the channel, deduplicating multi-channel chunk copies, sorting by chunk indices, and assembling complete `FileMetadata` objects with full MTProto provider references (`messageId`, `chatId`, `channelId`, `channelAccessHash`).
    3. Asynchronously persists the reconstructed registry back to the vault channel (`saveVaultRegistry`).
  - *Telegram-Backed Persistence (`TelegramAuthService.saveVaultRegistry`):* Automatically persists the JSON file registry into the user's private Telegram vault channel (as a silent text message if <3900 bytes or as a `bucketspace_registry.json` document attachment if larger).
