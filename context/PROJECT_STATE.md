...
- **Consumer UX + Hash Mismatch Fix** (Commits `5342401`, `1d1a7a0`, `cfbc0da`):
  ...
  - **Telegram Phone Flow & Global Country Code Picker**: Removed API ID, API Hash, Channel ID, MTProto, and Bot API terminology from user-facing UI. Built a searchable international country picker (`PhoneInputWithCountry`) with flags, dial prefixes (India `+91`, US `+1`, UK `+44`, UAE `+971`, etc.), and auto-detection on paste. Flow: Phone → Telegram Code → 2FA Password (only if required) → Connected.
