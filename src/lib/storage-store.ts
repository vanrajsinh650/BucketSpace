...
export class HttpTelegramStorageAdapter implements IStorageProvider {
  ...
  public async hasChunk(ref: import('@/shared').ProviderChunkRef): Promise<import('@/shared').ChunkStat> {
    const refObj = (ref.reference || {}) as Record<string, unknown>;
  ...
  public async deleteChunk(ref: import('@/shared').ProviderChunkRef): Promise<boolean> {
    const refObj = (ref.reference || {}) as Record<string, unknown>;
    const messageId = refObj.messageId;
